...
function Enquiry({ isAnimated, animKey }) {
  return (
    <section
      id="enquiry"
      className="scroll-mt-28 bg-[#FAF9F6] px-4 sm:px-6 lg:px-10 py-20 lg:py-28 border-t border-[#eadfcd]"
    >
      <div key={`enquiry-${animKey}`} className={`w-full max-w-7xl mx-auto ${isAnimated ? "section-card-enter" : ""}`}>
        ...
        <div className="grid gap-8 lg:gap-10 lg:grid-cols-[1.25fr_0.75fr] items-stretch">
          ...
          <aside className="relative overflow-hidden flex flex-col justify-between rounded-3xl border border-[#eadfcd] bg-white p-6 sm:p-8 md:p-10 shadow-sm hover:shadow-floating hover:border-brand-olive/50 transition-all duration-300 h-full group">
            {/* Subtle Animated Hover Overlay */}
...
function ProgramEnquiryForm() {
  ...
  return (
    <form
      ...
      className="rounded-3xl border border-[#e9e2d8] bg-white p-6 sm:p-8 md:p-10 shadow-sm hover:shadow-floating transition-all duration-300"
    >
    ...
    <label className="block">
      ...
      <div className="relative">
        <select
          ...
          required={required}
          defaultValue=""
          className="h-11 w-full appearance-none rounded-full border border-[#dcd8df] bg-white pl-4 pr-10 text-xs sm:text-sm font-medium text-slate-700 outline-none transition focus:border-brand-purple focus:ring-2 focus:ring-brand-purple/15 cursor-pointer"
        >
        ...
        <ChevronDown
          size={18}
          className="pointer-events-none absolute right-4 top-1/2 -translate-y-1/2 text-brand-purple/70"
        />
      </div>
    </label>
  );
